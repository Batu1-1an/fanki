...
export function FlashcardComponent({
  ...
  return (
    ...
    >
      ...
      <div 
        ref={cardRef}
        className="relative h-[70vh] min-h-[400px] max-h-[600px] sm:h-[480px] md:h-[520px] lg:h-[580px] perspective-1000"
        role="button"
            ...
            <CardContent className="p-4 sm:p-6 md:p-8 h-full flex flex-col">
              ...
              <div className="flex flex-col xs:flex-row xs:items-center justify-between mb-4 sm:mb-6 gap-3">
                <div className="flex items-center gap-2 sm:gap-3 min-w-0">
                  ...
                  <Badge 
                    variant="secondary"
                    className={cn("shrink-0", DIFFICULTY_LEVELS[currentWord.difficulty as keyof typeof DIFFICULTY_LEVELS]?.color)}
                  >
                    {DIFFICULTY_LEVELS[currentWord.difficulty as keyof typeof DIFFICULTY_LEVELS]?.label}
                  </Badge>
                  {currentWord.category && (
                    <Badge variant="outline" className="shrink-0 hidden sm:inline-flex">{currentWord.category}</Badge>
                  )}
                </div>
                <Button
                  variant="outline"
            ...
            <CardContent className="p-4 sm:p-6 md:p-8 h-full flex flex-col">
              ...
              <div className="flex flex-col xs:flex-row xs:items-center justify-between mb-4 sm:mb-6 gap-3">
                <div className="flex items-center gap-2 sm:gap-3">
                  ...
                  >
                    ...
                    <span className="text-xs sm:text-sm">Play Audio</span>
                  </Button>
                </div>
              ...
              <div className="flex-1 flex flex-col justify-center items-center space-y-6">
                ...
                <div className="text-center space-y-3 sm:space-y-4">
                  <h2 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl xl:text-6xl font-bold text-foreground break-words px-2">
                    {currentWord.word}
                  </h2>
                  
                  {currentWord.pronunciation && (
                    <p className="text-sm sm:text-base md:text-lg text-muted-foreground font-mono px-2">
                      /{currentWord.pronunciation}/
                    </p>
                  )}

                  {currentWord.definition && (
                    <p className="text-sm sm:text-base md:text-lg lg:text-xl text-muted-foreground max-w-xs sm:max-w-md mx-auto leading-relaxed px-4">
                      {currentWord.definition}
                    </p>
                  )}
                  ...
                  {currentWord.memory_hook && (
                    <div className="bg-blue-50 dark:bg-blue-950/30 border border-blue-200 dark:border-blue-800 rounded-lg p-3 max-w-xs sm:max-w-md mx-auto">
                ...
                <div className="w-full max-w-xs sm:max-w-md px-4">
                  {isGeneratingContent ? (
                    <div className="h-24 xs:h-32 sm:h-40 md:h-48 bg-muted rounded-lg flex items-center justify-center">
                    ...
                    <FlashcardImage 
                      ...
                      className="h-24 xs:h-32 sm:h-40 md:h-48 w-full"
                    />
                  ) : (
                    <div className="h-24 xs:h-32 sm:h-40 md:h-48 bg-muted rounded-lg flex items-center justify-center">
      ...
      <div className="mt-4 text-center text-xs sm:text-sm text-muted-foreground px-4">
        <div className="flex flex-wrap justify-center gap-2">
          <span>
            <kbd className="px-2 py-1 bg-muted rounded text-xs">Enter</kbd> submit
          </span>
          <span className="hidden sm:inline">•</span>
          <span>
            <kbd className="px-2 py-1 bg-muted rounded text-xs">Space</kbd> show
          </span>
          <span className="hidden sm:inline">•</span>
          <span>
            <kbd className="px-2 py-1 bg-muted rounded text-xs">1-4</kbd> rate
          </span>
        </div>
      </div>
