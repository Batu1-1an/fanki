...
serve(async (req)=>{
  ...
  try {
    ...
    // Parse request body to get word, difficulty, and userId
    const { word, difficulty, userId } = await req.json();
    // Validate that all required fields are present
    if (!word || !difficulty || !userId) {
      return new Response(JSON.stringify({
        error: 'Missing required fields: word, difficulty, userId'
      }), {
        status: 400,
        headers: {
          ...corsHeaders,
    ...
    // Define prompts based on difficulty level
...
Make the sentences engaging and educational for someone learning English.`;
    ...
    while(attempts < maxAttempts){
      try {
        // Fetch request to the Gemini 2.5 Flash model
        geminiResponse = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key=${geminiApiKey}`, {
          ...
          body: JSON.stringify({
            ...
            generationConfig: {
              ...
              topK: 40,
              topP: 0.95,
              // *** FIX: INCREASED TOKEN LIMIT AGAIN ***
    ...
    if (!generatedText) {
      console.error('Gemini response was missing content text. Response:', JSON.stringify(geminiData));
